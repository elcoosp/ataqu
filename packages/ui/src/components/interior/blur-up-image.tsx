...
export function BlurUpImage({
	...
	return (
		<div
			...
			className={`relative w-full overflow-hidden bg-muted dark:bg-white/15 ${className}`}
		>
			{placeholder ? (
			...
			{status === "error" ? (
				<motion.div
					...
					animate={{ opacity: 1 }}
					transition={transition}
					className="absolute inset-0 grid place-items-center bg-white text-muted-foreground dark:bg-card dark:text-muted-foreground"
				>

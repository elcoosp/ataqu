						...
						>
							{on ? (
								<motion.span
									...
									transition={CELL}
									className="absolute inset-0 rounded-[6px] bg-card dark:bg-background"
								/>
							) : null}

							<span
								aria-hidden
								className={`pointer-events-none absolute inset-0 rounded-[6px] border group-focus-visible:border-[var(--ring)] dark:group-focus-visible:border-[var(--ring)] ${
									on
										? "border-transparent"
										: "border-border dark:border-white/[0.16]"
								}`}
							/>
							<span className="relative col-start-1 row-start-1 inline-grid">
								<motion.span
									...
									transition={swap}
									className="col-start-1 row-start-1 inline-flex items-center gap-1.5 whitespace-nowrap text-[12.5px] font-medium text-secondary-foreground dark:text-secondary-foreground"
								>
									{filter.label}
									<span className="text-[10.5px] tabular-nums text-muted-foreground dark:text-muted-foreground">
										{counts[filter.id]}
									</span>
								</motion.span>
								<motion.span
									...
									transition={swap}
									className="col-start-1 row-start-1 inline-flex items-center gap-1.5 whitespace-nowrap text-[12.5px] font-medium text-foreground dark:text-foreground"
								>
									{filter.label}
			...
			<div className="relative mt-2.5">
					...
					>
						{visible.map((item) => (
							<motion.li
								...
								transition={step}
								className="min-w-0 overflow-hidden rounded-[11px] border border-border bg-white p-2.5 shadow-[0_1px_2px_rgba(28,25,23,0.06),0_4px_10px_-8px_rgba(28,25,23,0.45)] dark:border-white/[0.16] dark:bg-card dark:shadow-[0_1px_6px_rgba(0,0,0,0.45)]"
							>
								{renderItem(item)}
							</motion.li>
						))}
				...
				<AnimatePresence initial={false}>
					{visible.length === 0 && (
						...
						>
							<span className="text-[12.5px] text-muted-foreground dark:text-muted-foreground">
								{emptyLabel}
							</span>
						</motion.div>
					)}

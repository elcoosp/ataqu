...
describe("paginate (pure function)", () => {
	...
	it("middle: page=10 yields [1,'gap-l',9,10,11,'gap-r',20]", () => {
		expect(paginate(10, 20, 1, 1)).toEqual([
			1,
			"gap-l",
			9,
			10,
...
describe("usePagination", () => {
	...
	it("controlled mode with onPageChange", () => {
		...
		const { result, rerender } = renderHook(
			({ page }) => usePagination({ count: 10, page, onPageChange }),
			{ initialProps: { page: 1 } },
		);
		expect(result.current.page).toBe(1);
...
describe("useWizard", () => {
	it("next/back/goTo navigation", () => {
		const { result } = renderHook(() =>
			useWizard({ total: 4, defaultIndex: 0 }),
		);
	...
	it("first/last boundaries prevent overshoot", () => {
		const { result } = renderHook(() =>
			useWizard({ total: 3, defaultIndex: 0 }),
		);
		expect(result.current.isFirst).toBe(true);
		act(() => result.current.back());
		expect(result.current.index).toBe(0);
	...
	it("furthest tracks the highest visited index", () => {
		const { result } = renderHook(() =>
			useWizard({ total: 5, defaultIndex: 0 }),
		);
	...
	it("controlled index with onIndexChange", () => {
		...
		const { result, rerender } = renderHook(
			({ index }) => useWizard({ total: 3, index, onIndexChange }),
...
describe("useTabs", () => {
	...
	it("initial value and select", () => {
		const { result } = renderHook(() => useTabs({ items, defaultValue: "a" }));
	...
	it("controlled mode with onValueChange", () => {
		...
		const { result, rerender } = renderHook(
			({ value }) => useTabs({ items, value, onValueChange }),
...
describe("usePasswordStrength", () => {
	...
	it("strong password → full score with all rules met", () => {
		const { result } = renderHook(() => usePasswordStrength("MyS3cur3P@ss!"));
...
describe("usePollResults", () => {
	...
	it("vote sets chosen and computes shares", () => {
		const { result } = renderHook(() => usePollResults({ options }));
	...
	it("double vote is a no-op", () => {
		const onVote = vi.fn();
		const { result } = renderHook(() => usePollResults({ options, onVote }));
		act(() => result.current.vote("a"));
...
describe("useIconMorph", () => {
	...
	it("play-pause preset mode is fill", () => {
		const { result } = renderHook(() => useIconMorph({ preset: "play-pause" }));
	...
	it("onActiveChange fires on toggle", () => {
		...
		const { result } = renderHook(() => useIconMorph({ onActiveChange }));
		act(() => result.current.toggle());
...
describe("usePresence", () => {
	...
	it("summary text", () => {
		...
		expect(result.current.summary).toBe(
			"Person 0, Person 1 and 1 other are here",
		);
	});

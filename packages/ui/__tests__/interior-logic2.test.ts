...
describe("useAccordion", () => {
	...
	it("navigates headers with keyboard", () => {
		const { result } = renderHook(() =>
			useAccordion({ items, type: "single" }),
		);
		...
		const ev = (key: string) =>
			({ key, preventDefault: () => undefined }) as React.KeyboardEvent;
...
describe("useExpandingSearch", () => {
	it("expands, collapses and clears", () => {
		const onOpenChange = vi.fn();
		const { result } = renderHook(() => useExpandingSearch({ onOpenChange }));
...
describe("useTooltip", () => {
	it("opens via pointer after openDelay and closes after closeDelay", () => {
		...
		act(() =>
			result.current.triggerProps.onPointerLeave({} as React.PointerEvent),
		);
	...
	it("immediate open via focus, escape dismiss and cancel", () => {
		...
		act(() =>
			result.current.triggerProps.onPointerCancel({} as React.PointerEvent),
		);
		expect(result.current.open).toBe(false);
	});
	...
	it("dismiss blocks re-open until unblock, and skip/warm settle", () => {
		...
		act(() =>
			result.current.triggerProps.onPointerDown({} as React.PointerEvent),
		);
		act(() =>
			result.current.triggerProps.onPointerEnter({
				clientX: 200,
	...
	it("disabled hides open tooltip", () => {
		...
		act(() =>
			result.current.triggerProps.onPointerEnter({} as React.PointerEvent),
		);
		act(() => vi.advanceTimersByTime(200));
		expect(result.current.open).toBe(false);
	});
...
describe("usePopover", () => {
	...
	it("positions when refs are attached", () => {
		const { result } = renderHook(() =>
			usePopover({ open: true, side: "bottom", align: "center" }),
		);
...
describe("useContextMenu", () => {
	...
	it("getItemProps pointer move and click choose", () => {
		...
		act(() =>
			result.current.getItemProps(0).onClick({ detail: 1 } as React.MouseEvent),
		);
	...
	it("disabled does not open and touch hold works", () => {
		...
		act(() =>
			touch.current.triggerProps.onPointerMove({
				clientX: 12,
				clientY: 12,
			} as React.PointerEvent),
		);
		...
		act(() =>
			touch.current.triggerProps.onClick({
				preventDefault: () => undefined,
				stopPropagation: () => undefined,
			} as React.MouseEvent),
		);
...
describe("useModal", () => {
	it("closes via close(), Escape and backdrop", () => {
		...
		act(() =>
			result.current.overlayProps.onPointerDown({
	...
	it("exposes ids, target and panel props", () => {
		...
		expect(result.current.panelProps["aria-labelledby"]).toBe(

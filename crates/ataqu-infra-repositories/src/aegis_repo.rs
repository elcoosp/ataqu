...
impl AuthRepository for AegisUserRepository {
    ...
    async fn save_api_key(
        &self,
        key: &ataqu_domain_aegis::api_key::ApiKey,
    ) -> Result<(), AuthError> {
        let active = api_key_entity::ActiveModel {
            ...
            scopes: Set(serde_json::json!(key.scopes)),
    ...
    async fn find_api_key_by_hash(
        &self,
        hash: &str,
    ) -> Result<Option<ataqu_domain_aegis::api_key::ApiKey>, AuthError> {
    ...
    async fn list_api_keys(
        ...
        tenant_id: Uuid,
        user_id: Uuid,
    ) -> Result<Vec<ataqu_domain_aegis::api_key::ApiKey>, AuthError> {
        ...
        Ok(models
            .into_iter()
            .map(|m| ataqu_domain_aegis::api_key::ApiKey {
                id: m.id,
                tenant_id: ataqu_kernel::TenantId::new(m.tenant_id),
                user_id: m.user_id,
                name: m.name,

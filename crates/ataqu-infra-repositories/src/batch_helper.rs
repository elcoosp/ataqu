    ...
    Fut: std::future::Future<Output = Result<(), DbErr>> + Send,
{
    ...
    for chunk in items.chunks(chunk_size) {
        txn.execute_raw(Statement::from_sql_and_values(
            DbBackend::Postgres, "SAVEPOINT chunk_sp", []
        )).await?;

        match insert_fn(txn, chunk).await {
            Ok(_) => {
                txn.execute_raw(Statement::from_sql_and_values(
                    DbBackend::Postgres, "RELEASE SAVEPOINT chunk_sp", []
                )).await?;
                for item in chunk {
                    if let Ok(val) = serde_json::to_value(item) {
                        if let Some(id) = val.get("id").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()) {
                            successes.push(id);
                        }
                    }
                }
            ...
            Err(e) => {
                txn.execute_raw(Statement::from_sql_and_values(
                    DbBackend::Postgres, "ROLLBACK TO SAVEPOINT chunk_sp", []
                )).await?;
                ...
                for item in chunk {
                    txn.execute_raw(Statement::from_sql_and_values(
                        DbBackend::Postgres, "SAVEPOINT item_sp", []
                    )).await?;
                    match insert_fn(txn, std::slice::from_ref(item)).await {
                        Ok(_) => {
                            txn.execute_raw(Statement::from_sql_and_values(
                                DbBackend::Postgres, "RELEASE SAVEPOINT item_sp", []
                            )).await?;
                            if let Ok(val) = serde_json::to_value(item) {
                                if let Some(id) = val.get("id").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()) {
                                    successes.push(id);
                        ...
                        Err(e) => {
                            txn.execute_raw(Statement::from_sql_and_values(
                                DbBackend::Postgres, "ROLLBACK TO SAVEPOINT item_sp", []
                            )).await?;
    ...
    Ok(BatchResult { successes, failures })
}

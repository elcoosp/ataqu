...
use ataqu_domain_aegis::mfa::verify_totp;
...
pub use ataqu_domain_aegis::MfaSetupCompleted;
...
impl RealAegisDomain {
    ...
    pub fn authenticate(
        ...
        if user.mfa_enabled {
            ...
            if !verify_totp(secret, cmd.totp_code.as_ref().unwrap(), clock.now()) {
                return Err(AegisServiceError::AuthenticationFailed);
            }
        }
    ...
    pub fn verify_mfa(
        ...
        Ok(verify_totp(secret, code, clock.now()))
    }
    ...
    pub fn setup_mfa(
        ...
        user: &mut User,
        clock: &dyn Clock,
    ) -> Result<MfaSetupCompleted, AegisServiceError> {
        if user.mfa_enabled {
            return Err(AegisServiceError::Conflict(
                "MFA already enabled".to_string(),
            ));
        }
        user.mfa_secret = Some(cmd.totp_secret.clone());
        user.mfa_enabled = true;
        user.updated_at = clock.now();
...
impl AegisService {
    ...
    pub async fn setup_mfa(&self, user_id: Uuid) -> Result<MfaSetupResponse, AegisServiceError> {
        ...
        // Generate secret using rand
        use rand::RngCore;
        let mut bytes = [0u8; 20];
        rand::thread_rng().fill_bytes(&mut bytes);
        let secret = base32::encode(base32::Alphabet::RFC4648 { padding: false }, &bytes);
        let email_str = user.email.reveal(&ataqu_security::PiiAccessKey::new());
        let qr_code_url = ataqu_domain_aegis::mfa::generate_otpauth_url(&secret, email_str);
        let cmd = ataqu_domain_aegis::SetupMfaCommand {
            user_id,
            totp_secret: secret.clone(),
        };
        self.domain.setup_mfa(cmd, &mut user, self.clock.as_ref())?;
    ...
    pub async fn verify_mfa(&self, user_id: Uuid, code: &str) -> Result<(), AegisServiceError> {
        ...
        let user = self
            ...
            .await?
            .ok_or(AegisServiceError::NotFound("User not found".into()))?;
        if !self.domain.verify_mfa(&user, code, self.clock.as_ref())? {
            return Err(AegisServiceError::AuthenticationFailed);
        }
        let mut user = user;

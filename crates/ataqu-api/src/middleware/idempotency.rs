...
lazy_static::lazy_static! {
    static ref IDEMPOTENCY_CACHE: Cache<Uuid, (StatusCode, axum::http::HeaderMap, Vec<u8>)> = Cache::builder()
        .max_capacity(10_000)
...
pub async fn idempotency_middleware(mut req: Request, next: Next) -> Result<Response, StatusCode> {
            ...
            || req.method() == axum::http::Method::DELETE)
    {
            ...
            .map(|s| s.to_string())
        {
            ...
            if let Some((status, headers, body)) = IDEMPOTENCY_CACHE.get(&command_id) {

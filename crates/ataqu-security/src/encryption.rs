...
impl Encryptor {
    pub fn from_env() -> Self {
        let key_env = std::env::var("ENCRYPTION_KEY")
            .expect("ENCRYPTION_KEY must be set (32 bytes, base64)");
    ...
    pub fn decrypt(&self, combined_b64: &str) -> String {
        let combined = base64::engine::general_purpose::STANDARD.decode(combined_b64).unwrap();
        if combined.len() < 12 {

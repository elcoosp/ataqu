...
pub trait EmployeeRepositoryPort: Send + Sync {
    async fn insert(&self, tenant_id: &TenantId, event: &crate::EmployeeCreatedEvent) -> Result<(), PauseDomainError>;
...
pub trait LeaveRequestRepositoryPort: Send + Sync {
    async fn insert(&self, tenant_id: &TenantId, event: &crate::LeaveRequestedEvent) -> Result<(), PauseDomainError>;
    ...
    async fn update_status(&self, tenant_id: &TenantId, id: Uuid, status: LeaveStatus, reviewer_id: Uuid, updated_at: SystemTime) -> Result<(), PauseDomainError>;
}

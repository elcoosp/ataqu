...
fn evaluate_condition(cond: &Condition, payload: &serde_json::Value) -> bool {
    match cond {
        ...
        Condition::FieldContains { field, value } => payload
            .get(field)
            .and_then(|v| v.as_str())
            .map(|s| s.contains(value))
            .unwrap_or(false),
        Condition::FieldNotContains { field, value } => !payload
            .get(field)
            .and_then(|v| v.as_str())
            .map(|s| s.contains(value))
            .unwrap_or(true),
        Condition::FieldGreaterThan { field, value } => payload
            .get(field)
            .and_then(|v| v.as_f64())
            .map(|n| n > *value)
            .unwrap_or(false),
        Condition::FieldLessThan { field, value } => payload
            .get(field)
            .and_then(|v| v.as_f64())
            .map(|n| n < *value)
            .unwrap_or(false),
        Condition::FieldGreaterThanOrEqual { field, value } => payload
            .get(field)
            .and_then(|v| v.as_f64())
            .map(|n| n >= *value)
            .unwrap_or(false),
        Condition::FieldLessThanOrEqual { field, value } => payload
            .get(field)
            .and_then(|v| v.as_f64())
            .map(|n| n <= *value)
            .unwrap_or(false),
        ...
        Condition::FieldIn { field, values } => payload
            .get(field)

...
async function generateActionItems() {
	...
	console.log(
		`✅ ${topLeads.length} leads et ${topClusters.length} features générés.`,
	);
}
...
async function generateDailyBrief() {
	...
	const leads = pendingActions
		.filter((a) => a.type === "lead_outreach")
		.slice(0, 5);
	const features = pendingActions
		.filter((a) => a.type === "feature_build")
		.slice(0, 3);

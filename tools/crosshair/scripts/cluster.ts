...
import { clusters, insights } from "../src/db/schema";
...
async function createClusters() {
	...
	if (unclustered.length < 3) {
		console.log(
			"   ⏭️ Pas assez d'insights pour former un cluster (minimum 3).",
		);
		process.exit(0);
	}
	...
	for (const [competitor, items] of Object.entries(grouped)) {
		...
		const dataStr = items
			.map((i) => `- ${i.direct_quote || i.feature_gap || i.workflow}`)
			.join("\n");
		...
		try {
			...
			const existing = await db.query.clusters.findFirst({
				where: (fields, { and, eq }) =>
					and(eq(fields.name, result.name), eq(fields.mapped_app, competitor)),
			});
			...
			if (existing) {
				...
				const newScore = Math.round(
					((existing.total_opportunity_score || 0) + score) / 2,
				);
				await db
					.update(clusters)
					.set({
				...
				await db.insert(clusters).values({
					...
					core_pain:
						result.core_pain || items[0]?.feature_gap || "General pain",
					evidence_count: items.length,
					best_quotes: JSON.stringify(
						items.slice(0, 3).map((i) => i.direct_quote),
					),
			...
			console.log(
				`   ✅ Cluster créé: ${result.name} (${items.length} insights)`,
			);
		} catch (error) {
			console.error(
				`   ❌ Erreur de clustering pour ${competitor}:`,

...
import { insights, rawSignals } from "../src/db/schema";
		...
		for (const signal of signals) {
			...
			try {
				...
				if (!classification.is_complaint || classification.confidence < 0.7) {
					console.log(
						`   ⏭️ Not a complaint (conf=${classification.confidence.toFixed(2)}).`,
					);
					continue;
				}

				console.log(
					`   ✅ Complaint detected for ${classification.competitor}`,
				);
				...
				// 4. Calculate scores
				const leadScore =
					(leadData.urgency_score +
						leadData.buying_intent_score +
						leadData.frequency_score) /
					3;

				// 5. Determine if insight is high-value (has concrete data)
				const hasConcreteData =
					(productData.feature_gap && productData.feature_gap.length > 5) ||
					(productData.pricing_complaint &&
						productData.pricing_complaint.length > 5) ||
				...
				processed++;
				console.log(
					`   ✅ Done (Lead: ${leadScore.toFixed(1)}, Impact: ${productData.feature_impact_score})`,
				);
				if (!hasConcreteData) {
					console.log(
						`      ⚠️ Warning: no concrete feature/pricing/UX details extracted.`,
					);
				}
			} catch (error) {
				console.error(
					`   ❌ Error on signal ${signal.id}:`,

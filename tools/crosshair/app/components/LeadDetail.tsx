    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogHeader>
        <DialogTitle className="flex items-center justify-between">
          ...
          <span className={cn('text-lg font-semibold', scoreColor)}>Score: {score.toFixed(1)}</span>
        </DialogTitle>
      </DialogHeader>
      <DialogContent>
        {/* Author and Source */}
        <div className="flex items-center gap-4 text-sm text-gray-500">
          ...
          <span>•</span>
          <a href={signal.source_url || '#'} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
            View Source
          </a>
        </div>
        ...
        {signal.thread_context && signal.thread_context !== signal.raw_text && (
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Thread Context</h3>
            <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md text-sm whitespace-pre-wrap max-h-48 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Buyer Segment</h3>
          ...
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Current Workaround</h3>
          ...
          <div className="col-span-2">
            <h3 className="text-sm font-medium text-gray-500 mb-1">Direct Quote</h3>
            <p className="text-sm italic">"{lead.direct_quote || 'No quote'}"</p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Feature Gap</h3>
            <p className="text-sm">{lead.feature_gap || 'N/A'}</p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Pricing Complaint</h3>
            <p className="text-sm">{lead.pricing_complaint || 'N/A'}</p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">UX Friction</h3>
            <p className="text-sm">{lead.ux_friction || 'N/A'}</p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Feature Impact</h3>
        ...
        <div className="grid grid-cols-3 gap-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
          ...
          <div>
            ...
            <div className="text-lg font-semibold">{lead.buying_intent || 0}/10</div>
          </div>
          <div>
            <span className="text-xs text-gray-500">Frequency</span>
            <div className="text-lg font-semibold">{lead.frequency || 0}/10</div>
          </div>
        </div>

        {/* Custom Message */}
        <div>
          <h3 className="text-sm font-medium text-gray-500 mb-1">Suggested Message</h3>
          <textarea
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h3 className="text-sm font-medium text-gray-500 mb-1">Outreach Status</h3>
            <select
              value={status}
      ...
      <DialogFooter>
        ...
        <button
          onClick={() => {
            alert('Save changes (status & notes) - this will be persisted later');
            onOpenChange(false);
          }}

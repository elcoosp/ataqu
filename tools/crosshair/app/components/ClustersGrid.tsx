...
export function ClustersGrid({ clusters }: ClustersGridProps) {
  ...
  const [sortBy, setSortBy] = React.useState<'score' | 'evidence' | 'name'>('score');
  ...
  const sortedClusters = React.useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      if (sortBy === 'score') return (b.total_opportunity_score || 0) - (a.total_opportunity_score || 0);
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {apps.map(app => (
                <option key={app} value={app}>{app === 'all' ? 'All' : app}</option>
              ))}
            </select>
          </div>

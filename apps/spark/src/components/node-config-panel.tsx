...
const NATIVE_EVENTS = [
  ...
  { value: 'vault.stock.below_threshold', label: 'VAULT: Stock Below Threshold' },
  ...
  return (
    <aside className="w-80 border-l border-border bg-card p-4 overflow-y-auto flex-shrink-0">
      ...
      <div className="space-y-4">
        <div>
          <Label htmlFor="node-label"><Trans>Label</Trans></Label>
          <Input
            id="node-label"
            value={label}
        ...
        {nodeType === 'trigger' && (
          <>
            {subtype === 'event' && (
              <div>
                <Label htmlFor="event-type"><Trans>Outbox Event Type</Trans></Label>
                <select
                  id="event-type"
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateConfig('event_type', e.target.value)}
                >
                  <option value=""><Trans>Select an event...</Trans></option>
            ...
            {subtype === 'event' && (
              <div>
                <Label htmlFor="event-filter"><Trans>Filter (JSON, optional)</Trans></Label>
                <textarea
                  id="event-filter"
                  ...
                  value={typeof config.filter === 'string' ? config.filter : JSON.stringify(config.filter || {}, null, 2)}
            ...
            {subtype === 'schedule' && (
              <>
                <div>
                  <Label htmlFor="cron-expr"><Trans>Cron Expression</Trans></Label>
                  <Input
                    id="cron-expr"
                    value={String(config.cron || '')}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateConfig('cron', e.target.value)}
                    placeholder="0 9 * * 1-5"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateConfig('timezone', e.target.value)}
                  >
            ...
            {subtype === 'webhook' && (
              <>
                <div>
                  <Label><Trans>Webhook URL</Trans></Label>
                ...
                <div>
                  <Label htmlFor="webhook-secret"><Trans>Webhook Secret (optional)</Trans></Label>
                  <Input
                    id="webhook-secret"
                    type="password"
                    value={String(config.webhook_secret || '')}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateConfig('webhook_secret', e.target.value)}
                    className="mt-1 font-mono"
        ...
        {nodeType === 'action' && (
          <>
            <div>
              <Label htmlFor="action-params"><Trans>Parameters (JSON)</Trans></Label>
              <textarea
                id="action-params"
            ...
            {subtype === 'send_email' && (
              <div className="p-3 rounded-md bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground">
                  <Trans>Required params:</Trans> <code className="text-foreground">to</code>, <code className="text-foreground">subject</code>, <code className="text-foreground">body</code>
                </p>
              </div>
            )}
            {subtype === 'create_dial_channel' && (
              <div className="p-3 rounded-md bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground">
                  <Trans>Required params:</Trans> <code className="text-foreground">name</code>, <code className="text-foreground">channel_type</code>, <code className="text-foreground">participants</code>
                </p>
              </div>
            )}
            {subtype === 'send_dial_message' && (
              <div className="p-3 rounded-md bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground">
                  <Trans>Required params:</Trans> <code className="text-foreground">channel_id</code>, <code className="text-foreground">content</code>
                </p>
              </div>
            )}
            {subtype === 'reserve_vault_stock' && (
              <div className="p-3 rounded-md bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground">
                  <Trans>Required params:</Trans> <code className="text-foreground">variant_id</code>, <code className="text-foreground">quantity</code>
                </p>
              </div>
            )}
            {subtype === 'webhook_action' && (
              <div className="p-3 rounded-md bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground">
                  <Trans>Required params:</Trans> <code className="text-foreground">url</code>, <code className="text-foreground">method</code>, <code className="text-foreground">body</code>, <code className="text-foreground">headers</code>
                </p>
              </div>
            )}
          </>
        ...
        {nodeType === 'condition' && (
          <>
            {subtype !== 'and' && subtype !== 'or' && (
              <>
                <div>
                  <Label htmlFor="cond-field"><Trans>Field Path</Trans></Label>
                  <Input
                    id="cond-field"
                    value={String(config.field || '')}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateConfig('field', e.target.value)}
                ...
                {subtype !== 'field_exists' && (
                  <div>
                    <Label htmlFor="cond-value"><Trans>Value</Trans></Label>
                    <Input
                      id="cond-value"
              ...
              <div className="p-3 rounded-md bg-muted/30 border border-border">
                <p className="text-xs text-muted-foreground">
                  {subtype === 'and' ? (
                    ...
                    <Trans>At least one child condition must be true (OR logic)</Trans>
                  )}
                </p>
              </div>
            )}

	...
	if (workflows.length === 0) {
		return (
			<EmptyState
				...
				description={
					<Trans>
						Zapier would charge you $30/mo for this. Turn on your first native
						trigger.
					</Trans>
				}
			/>
				...
				>
					...
					<div className="flex items-center gap-3 ml-4">
						...
						<Switch
							...
							onCheckedChange={(checked: boolean) =>
								toggleMutation.mutate(
									{ id: wf.id, is_active: checked, version: wf.version },
									{
										onSuccess: () => {
											toast.success(
												checked ? t`Workflow enabled.` : t`Workflow disabled.`,
											);
										},
										onError: () => {
							...
							aria-label={
								wf.is_active ? t`Disable workflow` : t`Enable workflow`
							}
						/>
					</div>
				</div>

...
export function FormPreview({ form, open, onClose }: Props) {
  const { mode, currentSlide, answers, setAnswer, setCurrentSlide, reset } = useFormPreviewStore();
    ...
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="ataqu-glass flex h-[80vh] max-w-3xl flex-col overflow-hidden p-0">
        ...
        <div
          ...
          style={{ fontFamily: form.branding.fontFamily === 'jetbrains' ? 'JetBrains Mono, monospace' : 'Inter, sans-serif' }}
        >
          {submitted ? (

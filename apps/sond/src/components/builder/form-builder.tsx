...
function QuestionCard({ question, selected, onSelect, onDelete }: { question: SondQuestion; selected: boolean; onSelect: () => void; onDelete: () => void }) {
  const { attributes, listeners, setNodeRef } = useDraggable({ id: `q-${question.id}` });
  const { setNodeRef: setDropRef, isOver } = useDroppable({ id: `q-${question.id}` });
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected ? 'border-l-4 border-l-primary shadow-md' : 'border-border hover:border-primary/50',
...
export function FormBuilder({ questions, onUpdate }: Props) {
  ...
  const handleUpdateQuestion = (updates: Partial<SondQuestion>) => {
    if (!selectedId) return;
    onUpdate(questions.map((q) => (q.id === selectedId ? { ...q, ...updates } : q)));
  };
    ...
    <DndContext onDragEnd={handleDragEnd}>
      <div className="flex h-full">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div
            ref={setNodeRef}
            className={cn('flex-1 overflow-y-auto p-8 transition-colors', isOver && 'bg-primary/5')}
          >

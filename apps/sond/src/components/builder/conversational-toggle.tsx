		...
		<div className="flex items-center justify-between rounded-lg border border-border bg-card p-4">
					...
					<Tooltip>
						<TooltipTrigger asChild>
							<div className="cursor-help text-sm text-muted-foreground">
								{mode === "conversational"
									? t`Conversational (One per slide)`
									: t`Standard (All on one page)`}
							</div>
						</TooltipTrigger>
						<TooltipContent>
							<Trans>
								Conversational mode shows one question at a time — higher
								completion rates.
							</Trans>
						</TooltipContent>
					</Tooltip>
			...
			<Switch
				...
				onCheckedChange={(checked: boolean) =>
					onChange(checked ? "conversational" : "standard")
				}
			/>
		</div>
	);

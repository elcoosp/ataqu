...
export function StockAdjustment({ variant }: { variant: Variant }) {
  ...
  const adjustStock = useMutation<Variant, Error, AdjustStockVariables, AdjustStockContext>({
    ...
    onMutate: async (variables) => {
      ...
      queryClient.setQueriesData<PaginatedVariants>({ queryKey: ['vault', 'variants'] }, (old) => {
        if (!old) return old;
    ...
    onError: (_error, _variables, context) => {
      ...
      showToast({
        ...
        description: <Trans>The stock change was reverted. Please try again.</Trans>,
      });
    },
    onSuccess: () => {
      showToast({
    ...
    <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-border bg-card p-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="flex items-end">
          ...
          >
            {adjustStock.isPending ? <Trans>Adjusting...</Trans> : <Trans>Adjust Stock</Trans>}
          </Button>
        </div>
      </div>
    </form>

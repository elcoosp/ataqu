...
export function CreateProductForm({ onCreated }: { onCreated?: () => void }) {
  ...
  const mutation = useMutation<Product, Error, CreateProductRequest, CreateProductContext>({
    ...
    onMutate: async (variables) => {
      ...
      queryClient.setQueriesData<PaginatedProducts>({ queryKey: ['vault', 'products'] }, (old) => {
        if (!old) return old;
        return {
          ...old,
  ...
  const canSubmit = name.trim().length > 0 && sku.trim().length > 0 && !mutation.isPending;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border border-border bg-card p-4">
      ...
      <Button type="submit" disabled={!canSubmit}>
        {mutation.isPending ? <Trans>Creating...</Trans> : <Trans>Create Product</Trans>}
      </Button>
    </form>
  );
}

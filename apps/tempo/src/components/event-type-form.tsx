...
export function EventTypeForm({ eventType, onSuccess }: EventTypeFormProps) {
  ...
  const [durationMinutes, setDurationMinutes] = useState(eventType?.duration_minutes ?? 30);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch id="et-sms-reminder" checked={smsReminder} onCheckedChange={setSmsReminder} />
          </div>
          <div>
            <Label>
      ...
      <Button type="submit" disabled={isSubmitting}>
        {isSubmitting ? <Trans>Saving...</Trans> : <Trans>Save Event Type</Trans>}
      </Button>
    </form>
  );
}

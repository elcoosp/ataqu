...
import { Bone, Button, Card, useConfirm } from "@ataqu/ui";
...
export const Route = createFileRoute("/_auth/sond/")({
	...
	const handleDelete = useCallback(
		async (id: string, title: string) => {
			const ok = await confirm({
				title: t`Delete form`,
				description: t`Are you sure you want to delete "${title}"? This cannot be undone.`,
			...
			<div className="p-8">
				<div className="flex flex-col items-center justify-center py-16 text-center">
					...
					<Button
						className="mt-6"
						onClick={() =>
							navigate({ to: "/sond/builder/$id", params: { id: "new" } })
						}
					>
	...
	return (
		<div className="p-8">
			<div className="mb-8 flex items-center justify-between">
				...
				<Button asChild>
					<Link to="/sond/builder/$id" params={{ id: "new" }}>
					...
					>
						<Card className="p-6 transition-shadow hover:shadow-md relative">
							...
							<div className="text-sm text-muted-foreground">
								{f.mode === "conversational" ? t`Conversational` : t`Standard`}{" "}
								· v{f.version}
							</div>
						</Card>
					</Link>
